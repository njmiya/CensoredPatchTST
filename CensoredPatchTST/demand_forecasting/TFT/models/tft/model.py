...
class TemporalFusionTransformer(BaseModelWithCovariates):
        ...
        * non-linear variable processing in variable selection network can be shared among decoder and encoder
        ...
        if self.n_targets > 1:  # if to run with multiple targets
            self.output_layer = nn.ModuleList(
                [nn.Linear(self.hparams.hidden_size + len(self.hparams.extra_decoder), output_size) for output_size in
                 self.hparams.output_size]
            )
        else:
            self.output_layer = nn.Linear(self.hparams.hidden_size + len(self.hparams.extra_decoder),
                                          self.hparams.output_size)
    ...
    def forward(self, x: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if len(self.hparams.extra_decoder) > 0:
            extra_decoder = torch.cat([input_vectors[feat] for feat in self.hparams.extra_decoder], dim=-1)
            output = torch.cat([output, extra_decoder[:, max_encoder_length:, :]], dim=-1)
