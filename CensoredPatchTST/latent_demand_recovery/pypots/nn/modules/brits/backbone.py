...
class BackboneRITS(nn.Module):
    ...
    def forward(
        ...
        norm_val = inputs["norm_val"][:, 0, :]  # norm_val for normalization
        ...
        for t in range(self.n_steps):
            ...
            reconstruction_loss += self.loss_calc_func(x_h * norm_val, x * norm_val, m)

            x_c = m * x + (1 - m) * x_h

            z_h = self.feat_reg(x_c)
            reconstruction_loss += self.loss_calc_func(z_h * norm_val, x * norm_val, m)

            alpha = torch.sigmoid(self.combining_weight(torch.cat([gamma_x, m], dim=1)))

            c_h = alpha * z_h + (1 - alpha) * x_h
            reconstruction_loss += self.loss_calc_func(c_h * norm_val, x * norm_val, m)
...
class BackboneBRITS(nn.Module):
    ...
    def __init__(
        ...
        self.loss_calc_func = loss_calc_func
        # create models
        self.rits_f = BackboneRITS(n_steps, n_features, rnn_hidden_size, self.loss_calc_func)
        self.rits_b = BackboneRITS(n_steps, n_features, rnn_hidden_size, self.loss_calc_func)
    ...
    def _get_consistency_loss(
        ...
        loss = self.loss_calc_func(pred_f, pred_b, torch.ones_like(pred_f).to(torch.float32)) * 1e-1
        return loss
    ...
    def forward(self, inputs: dict) -> Tuple[torch.Tensor, ...]:
        ...
        consistency_loss = self._get_consistency_loss(f_imputed_data * norm_val, b_imputed_data * norm_val)
