...
def calc_reg_focal(
    ...
    bias = targets - predictions
    bias_idx = bias < 0  # predictions is higher than targets
    # if masks is not None:
    #     mse = lib.square(bias) * masks
    #     skew = (lib.sum(mse[~bias_idx]) + 1) / (lib.sum(mse[bias_idx]) + 1)
    #     skew = lib.clip(skew, 0.3, 3)
    #     reg_focal_loss = skew ** gamma * mse * ~bias_idx + (1 / skew) ** gamma * mse * bias_idx
    #     return lib.sum(reg_focal_loss) / lib.sum(masks)
    # else:
    #     mse = lib.square(bias)
    #     skew = lib.sum(mse[~bias_idx] + 1e-5) / lib.sum(mse[bias_idx] + 1e-5)
    #     skew = lib.clip(skew, 0.3, 3)
    #     reg_focal_loss = skew ** gamma * mse * ~bias_idx + (1 / skew) ** gamma * mse * bias_idx
    #     return lib.mean(reg_focal_loss)
    if masks is not None:
        mse = lib.square(bias) * masks
        nmse = lib.sum(mse[~bias_idx])
        pmse = lib.sum(mse[bias_idx])
        return lib.sum(mse) / lib.sum(masks) + gamma * lib.abs(lib.sum(bias * masks) / lib.sum(masks))
    else:
        mse = lib.square(bias)
        skew = lib.sum(mse[~bias_idx] + 1e-5) / lib.sum(mse[bias_idx] + 1e-5)
        skew = lib.clip(skew, 0.3, 3)
...
def calc_mbe(
    ...
    if masks is not None:
        return lib.abs(lib.sum(bias * masks) / (lib.sum(masks) + 1e-12))
    else:
        return lib.abs(lib.mean(bias))
