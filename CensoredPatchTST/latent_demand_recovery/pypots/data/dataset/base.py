...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        if isinstance(self.data, str):  # data from file
            ...
            if self.return_static:
                assert (
                        "static_var" in self.file_handle.keys()
                ), "The given dataset file doesn't contains static_var. Please double check."
            ...
            if self.return_static:
                assert (
                        "static_var" in self.data.keys()
                ), "The given dataset dictionary doesn't contains static_var. Please double check."
            ...
            static_var = None if "static_var" not in data.keys() else data["static_var"]
            norm_val = np.ones(X.shape) if "norm_val" not in data.keys() else data["norm_val"]
            self.X, self.X_ori, self.X_pred, self.y, self.static_var, self.norm_val = self._check_array_input(
                X, X_ori, X_pred, y, static_var, norm_val, "tensor"
            )
    ...
    def _get_data_sizes(self) -> Tuple[int, ...]:
        ...
        n_samples, n_steps, n_features, n_statics, n_pred_steps, n_pred_features = 0, 0, 0, 0, 0, 0
        ...
        if isinstance(self.data, str):
            ...
            if self.return_static:
                first_static_sample = self.file_handle["static_var"][0]
                n_statics = first_static_sample.shape[-1]
        else:
            ...
            if self.return_static:
                n_statics = self.static_var[0].shape[-1]

        return n_samples, n_steps, n_features, n_statics, n_pred_steps, n_pred_features
    ...
    def _check_array_input(
            ...
            static_var: Optional[Union[np.ndarray, torch.Tensor]] = None,
            norm_val: Optional[Union[np.ndarray, torch.Tensor]] = None,
        ...
        assert X_shape[0] % self.graph_size == 0, (
            f"input length should be an integer multiple of the graph size"
        )
        ...
        if static_var is not None:
            assert len(X) == len(static_var), (
                f"lengths of X and static_var must match, " f"but got f{len(X)} and {len(static_var)}"
            )
            static_var = turn_data_into_specified_dtype(static_var, out_dtype)
            static_var = static_var.to(torch.long) if out_dtype == "tensor" else static_var

        if norm_val is not None:
            assert len(X) == len(norm_val), (
                f"lengths of X and norm_val must match, " f"but got f{len(X)} and {len(norm_val)}"
            )
            assert X.shape[-1] == norm_val.shape[-1], (
                f"features number of X and norm_val must match, " f"but got f{X.shape[-1]} and {norm_val.shape[-1]}"
            )
            norm_val = turn_data_into_specified_dtype(norm_val, out_dtype)
            norm_val = norm_val.to(torch.float32) if out_dtype == "tensor" else norm_val

        tensors = (X, X_ori, X_pred, y, static_var, norm_val)
        if self.graph_size > 1:
            graph_num = X_shape[0] // self.graph_size
            tensors = map(lambda x: x.reshape(graph_num, self.graph_size, x.shape[1:]), tensors)
        return tensors
    ...
    def _fetch_data_from_array(self, idx: int) -> Iterable:
        ...
        if self.return_static:
            sample.append(self.static_var[idx].to(torch.long))

        sample.append(self.norm_val[idx])
    ...
    def _fetch_data_from_file(self, idx: int) -> Iterable:
        ...
        if self.return_static:
            sample.append(self.file_handle["static_var"][idx].to(torch.long))

        if "norm_val" in self.file_handle.keys():
            sample.append(self.file_handle["norm_val"][idx])
        else:
            sample.append(torch.ones_like(X).to(torch.float32))
