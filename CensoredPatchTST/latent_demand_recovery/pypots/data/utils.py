...
def _parse_delta_torch(missing_mask: torch.Tensor) -> torch.Tensor:
    ...
    if len(missing_mask.shape) == 2:
        ...
        d = [torch.zeros(n_samples, 1, n_features, device=device)]
        for step in range(1, n_steps):
            d.append(
                torch.ones(n_samples, 1, n_features, device=device) + (1 - missing_mask[:, step - 1: step, :]) * d[-1])

        delta = torch.concat(d, dim=1)
